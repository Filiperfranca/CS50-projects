...
def index():
    ...
    user_habits = db.execute("SELECT * FROM habits WHERE user_id = ? AND id NOT IN (SELECT habit_id FROM history WHERE date = CURRENT_DATE)", session["user_id"]) #we pull user-created habits
...
def complete():
    ...
    db.execute("INSERT INTO history (habit_id) VALUES (?)", habit_id)
